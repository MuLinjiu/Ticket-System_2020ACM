...
#define ClockTest(x)   do{if(functionToBeTimed())*functionToBeTimed() += clock() - tClockTimer();tClockTimer() = clock();functionToBeTimed() = &time_recorder()[x];++function_called_num()[x];}while(0)
#define ResetClock     ClockTest(__FUNCTION__)
#define StartStopWatch ClockTest("STOP_WATCH")
